...
func init() {
	http_server.RegisterRoutes(func(r chi.Router, c *core.Container) error {
		...
		r.Route("/api/v1/atlas-kg", func(r chi.Router) {
			...
			// ====== Atlas Sample Run (async sampling task + progress + per-category review) ======
			r.Post("/sample-runs", kgCtrl.CreateSampleRun)
			r.Get("/sample-runs", kgCtrl.ListSampleRuns)
			r.Get("/sample-runs/{run_id}", kgCtrl.GetSampleRun)
			r.Post("/sample-runs/{run_id}/progress", kgCtrl.UpdateSampleRunProgress)
			r.Put("/sample-runs/{run_id}/status", kgCtrl.UpdateSampleRunStatus)
			r.Get("/sample-runs/{run_id}/category-results", kgCtrl.ListSampleCategoryResults)
			r.Get("/sample-runs/{run_id}/category-results/{report_type}", kgCtrl.GetSampleCategoryResult)
			r.Put("/sample-runs/{run_id}/category-results/{report_type}", kgCtrl.UpsertSampleCategoryResult)
			r.Put("/sample-runs/{run_id}/category-results/{report_type}/field-summary", kgCtrl.UpdateSampleFieldSummary)
			r.Get("/sample-runs/{run_id}/document-results", kgCtrl.ListSampleDocumentResults)
			r.Post("/sample-runs/{run_id}/document-results", kgCtrl.CreateSampleDocumentResult)
			r.Put("/sample-runs/{run_id}/document-results/{doc_id}", kgCtrl.UpdateSampleDocumentResult)
