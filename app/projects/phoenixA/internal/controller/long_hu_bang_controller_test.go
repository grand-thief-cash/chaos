...
func TestLongHuBangJSONDeserialization(t *testing.T) {
	artemisPayload := `[{"security_id":1,"source":"amazing_data","trade_date":"2026-05-27","security_name":"平安银行","reason_type":"1001","reason_type_name":"日涨幅偏离值达7%","trader_name":"国泰君安证券上海分公司","flow_mark":1,"change_range":9.98,"buy_amount":123456789.12,"sell_amount":98765432.10,"total_amount":24680246.80,"total_volume":321.50}]`
	...
	if rec.SecurityID != 1 {
		t.Fatalf("expected security_id=1, got %d", rec.SecurityID)
	}
	if rec.FlowMark != 1 {
...
func TestLongHuBangSecurityIDAndDateNormalization(t *testing.T) {
	payload := `[{"security_id":1,"trade_date":"20260527","security_name":"平安银行","reason_type":"1001","reason_type_name":"日涨幅偏离值达7%","trader_name":"国泰君安证券上海分公司","flow_mark":2,"change_range":-5.2,"buy_amount":100.0,"sell_amount":200.0,"total_amount":300.0,"total_volume":10.0}]`
	...
	if list[0].SecurityID != 1 {
		t.Fatalf("expected security_id=1, got %d", list[0].SecurityID)
	}
