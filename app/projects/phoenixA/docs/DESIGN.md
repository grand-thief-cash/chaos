...
### Phase 1 补充：对接 Artemis 历史回测 MVP 的 implementation checklist

这一部分是对上面 Phase 1 的补充，不是另起一个新 Phase。它表达的是：当 Artemis 开始接入历史回测任务时，PhoenixA 侧最小需要补到什么程度，才能稳定支撑第一次联调。

#### A. 边界与命名冻结
- [ ] PhoenixA 继续只负责“数据读取 + 结果持久化 + 结果查询”，不负责回测执行逻辑
- [ ] 命名建议直接冻结为：
  - `StrategyRunSummary`
  - `StrategyRunArtifact`
- [ ] Phase 1 不引入 `CampaignSummary` 表
- [ ] Phase 1 不引入 plot 图片二进制存储，只保留 plot 数据 artifact 能力

#### B. 历史 bars 读取能力
- [ ] 确认 Artemis 是否直接复用现有历史数据接口，还是新增一个面向策略回测的统一 bars 接口
- [ ] 无论最终路由如何，响应结构必须稳定支持：
  - `symbol`
  - `trade_date` 或标准化时间字段
  - `open/high/low/close`
  - `volume`
  - `amount`（若有）
- [ ] 支持 Artemis 按 `symbol + timeframe + date_range` 查询历史 bars
- [ ] 对返回顺序做明确约束：默认按时间升序
- [ ] 对最大返回条数做安全限制，避免单次把过大数据集直接打穿接口

#### C. run summary / artifact 持久化模型
- [ ] 增加 `StrategyRunSummary` 对应 model / dao / service
- [ ] 增加 `StrategyRunArtifact` 对应 model / dao / service
- [ ] `StrategyRunSummary` 至少覆盖以下字段：
  - `run_id`
  - `parent_run_id`
  - `task_code`
  - `mode`
  ...
  - `error_message`
  - `duration_ms`
- [ ] `StrategyRunArtifact` 至少覆盖以下字段：
  - `run_id`
  - `artifact_type`
  - `payload_json`
  - `payload_version`
  - `created_at`
- [ ] Phase 1 支持的 `artifact_type` 白名单建议至少包含：
  - `analyzers`
  - `trades`
  - `equity_curve`
  - `plot_manifest`
...
#### D. 索引与查询最小要求
- [ ] `StrategyRunSummary.run_id` 唯一
- [ ] 为 `parent_run_id` 建索引，便于一个 campaign 下查多个 child run
- [ ] 为 `strategy_code`、`symbol`、`created_at` 建常用查询索引
- [ ] `StrategyRunArtifact` 至少保证能按 `run_id` 高效查询
- [ ] 如 artifact 数据量预期较大，可补 `run_id + artifact_type` 复合索引

#### E. API / Controller / Service / DAO checklist
- [ ] 增加写入接口：
...
- [ ] Controller 只做请求解析、基础参数校验、错误码映射
- [ ] Service 负责：
  - upsert 规则
  - artifact type 白名单校验
  - payload 基础完整性校验
- [ ] DAO 只负责表读写，不承载 artifact 业务逻辑判断

#### F. Artemis 联调支持项
- [ ] 明确给 Artemis 的 bars query contract，避免 Artemis 侧自己猜字段含义
- [ ] 明确给 Artemis 的 summary/artifact upsert contract，避免两边各自定义 payload
- [ ] 至少准备一份联调基线数据集，确保 Artemis 在开发期能重复跑同一结果
- [ ] 错误响应要尽量结构化，便于 Artemis 记录 `run_id` 级别的失败原因
...
- [ ] Artemis 能稳定读取 PhoenixA 历史 bars，并拿到一致字段结构。
- [ ] Artemis 能写入 1 条 `StrategyRunSummary` 与多条 `StrategyRunArtifact`。
- [ ] PhoenixA 能按 `run_id` 查询完整回测结果。
- [ ] 非法 `artifact_type` 或缺字段 payload 会被 PhoenixA 拒绝并给出可诊断错误。
