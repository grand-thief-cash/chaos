...
| GET | `/api/v2/securities/{symbol}` | 获取单个证券信息 |
| GET | `/api/v2/securities/count` | 统计证券数量 |
| POST | `/api/v2/securities/upsert` | 批量插入/更新证券信息 |
...
| name | string | 证券名称 |
| full_name | string | 证券全称（可选） |
| status | string | 状态（active, delisted 等） |
| list_date | string | 上市日期（格式 YYYY-MM-DD，可选） |
| delist_date | string | 退市日期（格式 YYYY-MM-DD，可选） |
