...
CREATE TABLE IF NOT EXISTS ods.financial_statement (
    ...
    created_at       TIMESTAMPTZ    NOT NULL DEFAULT NOW(),
    updated_at       TIMESTAMPTZ    NOT NULL DEFAULT NOW(),
    CONSTRAINT uk_fin_stmt UNIQUE (security_id, source, statement_type, reporting_period, report_type, statement_code),
...
CREATE TABLE IF NOT EXISTS ods.corporate_action (
    ...
    created_at       TIMESTAMPTZ    NOT NULL DEFAULT NOW(),
    updated_at       TIMESTAMPTZ    NOT NULL DEFAULT NOW(),
    CONSTRAINT uk_corp_action UNIQUE (security_id, source, action_type, report_period, ann_date),
...
COMMENT ON COLUMN ods.long_hu_bang.created_at IS '记录创建时间（Phase 3 新增）。';
COMMENT ON COLUMN ods.long_hu_bang.updated_at IS '记录更新时间（Phase 3 新增）。';
...
CREATE INDEX IF NOT EXISTS idx_es_current_valid
    ON ods.equity_structure (source, current_sign, is_valid)
    TABLESPACE warm_storage
