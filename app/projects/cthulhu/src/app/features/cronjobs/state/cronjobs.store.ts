...
export class CronjobsStore {
  ...
  pagedTasks = computed(()=> this.filteredTasks()); // value (array) for template
  ...
  loadTasks(force = true){
    ...
    const cr = this._createdRange(); if(cr.from) q.created_from = cr.from; if(cr.to) q.created_to = cr.to;
    const ur = this._updatedRange(); if(ur.from) q.updated_from = ur.from; if(ur.to) q.updated_to = ur.to;
    this.api.listTasks(q).subscribe({
      next: (resp: TaskListResponse)=> { console.debug('[CronjobsStore] loadTasks resp', resp); this._tasks.set(resp.items); this._taskTotal.set(resp.total); },
  ...
  loadRuns(taskId: number, force = false){
    ...
    this.api.listRuns(taskId).subscribe({
      next: data => { console.debug('[CronjobsStore] loadRuns resp', taskId, data); this._runs.set({ ...this._runs(), [taskId]: data }); },
  ...
  // Reactive helpers (signals returning values for a task)
  ...
  pagedRuns(taskId: number){ return computed(()=> {
    const list = this._runs()[taskId] || [];
    const page = this._runPageIndex()[taskId] || 1;
    const size = this._runPageSize()[taskId] || 10;
