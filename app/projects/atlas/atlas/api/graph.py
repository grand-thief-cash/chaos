...
from atlas.services.graph_service import (
    ...
    get_competitors, search_nodes, get_event_impacts,
)
...
async def get_chain(name: str, max_hops: int = Query(3, ge=1, le=5)):
    ...
    data = get_company_chain(name, max_hops=max_hops)
...
async def get_timeline(name: str):
    ...
    data = get_company_timeline(name)
