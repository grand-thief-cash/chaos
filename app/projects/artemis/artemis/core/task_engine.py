...
class TaskEngine:
    ...
    def _execute(self, task_code: str, incoming_params: dict, headers: dict | None, async_mode: bool = False):
        ...
        # normalize exec_type early so ctx.exec_type 不会是 None
        ctx.exec_type = str((ctx.exec_type or ('ASYNC' if async_mode else 'SYNC'))).upper()
        ...
        def run_unit():
            try:
                ...
                try:
                    ...
                    cb = ctx.callback
                    if async_mode and is_top_level and cb and not cb.finalized():
        ...
        if async_mode and is_top_level and ctx.callback and not ctx.callback.finalized():
            try:
                if success and ctx.status == TaskStatus.SUCCESS.value:
                    ...
                    if ctx.logger:
                        ctx.logger.info({'event':'callback_finalize_success','task_code':task_code,'run_id': ctx.run_id})
                # 失败的情况已经在 run_unit 的 except 块里处理过了
            except Exception as fe:
                if ctx.logger:
                    ctx.logger.warning({'event':'callback_finalize_success_error','error':str(fe),'task_code':task_code,'run_id': ctx.run_id})
