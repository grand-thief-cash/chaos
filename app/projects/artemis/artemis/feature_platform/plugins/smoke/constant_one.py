...
class ConstantOneFeature:
    """Control-plane smoke plugin. It has no research meaning."""

    def validate(self, definition: dict, version: dict, implementation: dict) -> None:
        if definition.get("value_type") != "number":
            raise FeaturePlatformError("INPUT_SCHEMA_INVALID", "constant_one requires number value_type")

    def load_inputs(self, ctx: FeatureExecutionContext, provider, dependencies: list[dict]):
        if dependencies:
            raise FeaturePlatformError("DEPENDENCY_REFERENCE_INVALID", "constant_one must not have dependencies")
        return None
        ...
        return FeatureNumericOutput(
            ...
            rows=[
                NumericValue(
                    ...
                    value=1.0,
                    value_status="valid",
                    quality_flags={"smoke": "constant_one"},
                )
    ...
    def validate_output(self, ctx: FeatureExecutionContext, output: FeatureNumericOutput) -> None:
        if len(output.rows) != len(ctx.security_ids):
            raise FeaturePlatformError("OUTPUT_SCHEMA_INVALID", "constant_one must output every subject")
