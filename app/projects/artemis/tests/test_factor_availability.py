...
def test_factor_service_availability_uses_capabilities(monkeypatch):
    monkeypatch.setattr(factor_service, "_get_catalog_capabilities", lambda refresh=False: _sample_capabilities_payload())
...
def test_factor_service_availability_marks_missing_sources(monkeypatch):
    ...
    monkeypatch.setattr(factor_service, "_get_catalog_capabilities", lambda refresh=False: payload)
...
def test_factor_availability_route(monkeypatch):
    ...
    monkeypatch.setattr(factor_service, "get_availability", lambda refresh=False: {"refresh": refresh, "factors": []})

    client = TestClient(app)
    response = client.get("/factors/availability", params={"refresh": "true"})

    assert response.status_code == 200
    assert response.json() == {"refresh": True, "factors": []}
