...
| **Net Income CAGR 3Y** | `(NI_TTM(t) / NI_TTM(t-12Q))^(1/3) - 1` | 3 年净利润复合增长 |
...
| **Cash/ST Debt** | `CURRENCY_CAP / (ST_BORROWING + NONCUR_LIAB_DUE_WITHIN_1Y)` | 现金覆盖短期债务 |
...
> - 因子引擎调用 PhoenixA Bars API 时，`adjust` 现在只由 factor catalog 中的 `market_adjust_policy.adjust` 决定，不再通过运行期配置做默认/覆盖
> - 因此每个依赖 market bars 的因子都必须在 catalog 中显式声明自己的口径；若缺失，pipeline 会直接报错，避免“默认值偷偷生效”
> - `phoenix_queries` 只描述会调用哪个 PhoenixA endpoint；具体使用 `nf / qfq / hfq` 由 `market_adjust_policy` 单独治理，避免重复表达
> 
> ⚠️ **股息口径治理说明**：
...
>
> ⚠️ **行业分类治理说明**：
> - 因子引擎直接消费 PhoenixA `GET /api/v2/taxonomy/by_security/{symbol}` 返回的 `canonical_*` 与 `derived_flags`
> - `derived_flags.financial_sector` 是金融行业判定的唯一 taxonomy 语义来源；Artemis 不再基于行业代码前缀维护独立 fallback
>
> **EV（企业价值）详细定义**：
> ```
...
| ST / *ST 股票 | 正常计算，但打标 `is_st=true`，策略决定是否过滤 |
...
| 修订点 | 原版问题 | 修正内容 |
|--------|---------|---------|
| **TTM/单季度** | 未说明中国财报累计值特性 | 新增 4.1 累计值说明 + 4.3 单季度推导 |
