...
> 日期：2026-08-18
> 状态：两股 46 股票日的不对称 BUY/SELL 规则基线已完成；数据扩样与文件型 Level-1 pilot 已落地；两阶段模型、30 股滚动样本外和 LOSO/LOIO 尚未执行
...
6. **规则层 BUY/SELL 已拆开，但仍只是探索性 baseline。** 扩至生益科技、长江电力各 23 个
   交易日后，BUY gate 在两股都有效过滤不利下跌候选；SELL 改为下跌趋势中的反弹失败，
   生益科技 5/15 分钟均为正，长江电力仅有 12 个且 15 分钟中位数仍负。它支持“不对称、
   先判机会、允许不发信号”，不支持“已经会抓深 V/每个单边日都能倒 T”。
...
| 单边门控 MACD + 量能 + EMA，BUY 5 分钟 | `n=18`，正确率 61.11%，平均 +0.3152%，MFE/MAE 0.7580%/0.2926%；旧规则同期 `n=56`、46.43%、+0.1028% | abstention 有效；不是正式 OOS |
| 同一门控，SELL 5 分钟 | `n=17`，正确率 35.29%，平均 -0.1778%，MAE 0.6408% | SELL 仍失败，必须独立建模 |
| 两股扩样后的不对称规则，生益科技 BUY 5/15 分钟 | `n=29`，5 分钟 55.17% / +0.3059%；15 分钟 55.17% / -0.0317%，中位数 +0.0926% | 5 分钟改善，15 分钟 horizon 脆弱 |
| 两股扩样后的不对称规则，生益科技 SELL 5/15 分钟 | `n=54`，62.96% / +0.1090%；`n=52`，71.15% / +0.2773% | 独立 SELL 规则有希望；同区间设计，非 OOS |
| 两股扩样后的不对称规则，长江电力 BUY 5/15 分钟 | `n=26`，61.54% / +0.0567%；69.23% / +0.1501% | 方向为正，但 5 分钟绝对空间小 |
| 两股扩样后的不对称规则，长江电力 SELL 5/15 分钟 | `n=12`，75.00% / +0.1393%；41.67% / +0.0536%，15 分钟中位数 -0.0524% | 机会门槛有效降频，长路径不稳定 |
...
本轮把外部建议改写成可因果复核的 `macd_volume_regime_reversal_v1`，没有引入任何事后
V 形函数。普通候选仍来自 EMA/ATR 深偏离、MACD 柱转折与近期量能；只有价格低于 session
VWAP 且慢 EMA、VWAP 的 N-bar 斜率同时向下时，BUY 才进入额外门控。门控四项为：N-bar
急跌、当前量相对此前 N bar 均量的恐慌倍数、同日价格新低而 MACD histogram/DIF 不新低、
连续阳线收复近期最大阴线实体。默认至少 3/4；SELL 在不利上涨状态中镜像。信号返回每项
原始值、布尔结果和总分。

测试区间为生益科技 2026-08-01 至 08-14，其中 10 个交易日有数据；配置沿用用户给出的
`EMA 5/13、MACD signal 4、0.35 ATR、MACD 收敛 2 bars、基础量比 0.8、每侧上限 10`，
新增门控默认 `5 bars、2%、3x、20-bar divergence、3 连续 bars、收复 50%、3/4`。主结果：

| 5 分钟事件 | 旧规则 | 单边门控规则 |
|---|---:|---:|
| 全部可评估信号 | 115 | 35 |
| 全部正确率 / 平均方向收益 / edge | 40.87% / -0.0319% / 0.9133 | 48.57% / +0.0758% / 1.1813 |
| BUY 数 / 正确率 / 平均方向收益 | 56 / 46.43% / +0.1028% | 18 / 61.11% / +0.3152% |
| BUY 平均 MFE / MAE | 0.5594% / 0.3939% | 0.7580% / 0.2926% |
| SELL 数 / 正确率 / 平均方向收益 | 59 / 35.59% / -0.1598% | 17 / 35.29% / -0.1778% |

新规则 BUY 在 15 分钟也是 `n=18、61.11%、平均 +0.4575%`。逐日观察中，8 月 13 日旧规则
发出 10 个 BUY，新规则全部 veto；8 月 14 日旧规则发出 8 个 BUY，新规则只在 14:25 价格
重新位于 VWAP 上方且 VWAP 斜率转正后发出一个 BUY，该点未来 5 分钟 `+0.3136%`，但未来
15 分钟为 `-1.3033%`，再次说明“点位有效”依赖预测 horizon。

阈值敏感性没有支持放宽默认值：`1%/2x/3-of-4` 的 BUY 为 `n=24、50.00%、+0.1311%`；
`0.8%/1.5x/3-of-4` 为 `n=32、46.88%、+0.1659%`；`1%/2x/2-of-4` 为
`n=39、56.41%、+0.2634%`。严格默认的 BUY 结果最好，但 18 个 BUY 全部发生在没有不利
状态门控的时段，**没有一个单边下跌 BUY 真正以 3/4 证据获准**。因此可信结论只有：硬 veto
减少了错误抄底；这 10 日不存在规则能够证明的“深 V 买点”。SELL 侧未改善，不能用 BUY
结果为其背书。该区间用于实现调试和阈值敏感性读取，不是 untouched OOS，下一步仍必须执行
30 股 walk-forward、LOSO/LOIO 与匹配随机时点协议。

#### 2.3.8 两股扩样与不对称 SELL 规则

用户观察到第一版门控策略“可能不如旧策略”，这个判断在合并 BUY/SELL 后成立：第一版
SELL 使用 BUY 的镜像四维门控，在 23 日生益科技扩样中 5 分钟为
`n=51、41.18%、-0.2016%`，掩盖了 BUY 的改善。于是本轮没有继续给镜像规则调阈值，而是把
SELL 重新定义为倒 T 的独立问题。

实验范围固定为 2026-07-15 至 2026-08-14，生益科技（`4889/600183`）和长江电力
（`4446/600900`）各 23 个有数据交易日，共 46 个股票日；16 个周末按无数据跳过，API 失败
为 0。两股形成刻意的波动对照：生益科技平均单日 high/low 振幅 8.5710%，长江电力为
2.2464%。策略参数沿用 `EMA 5/13、MACD signal 4、基础量比 0.8、cooldown 5、每侧上限
10`，主评估冻结在未来 5/15 根 min1 bar，不启用成交模拟。

BUY 规则没有改变。最直接的反事实是把旧 BUY 按新 gate 是否保留分组：

| 股票 | 旧 BUY 被保留 | 旧 BUY 被删除 |
|---|---:|---:|
| 生益科技 5 分钟 | `n=28 / 57.14% / +0.3228%` | `n=131 / 35.88% / -0.0794%` |
| 长江电力 5 分钟 | `n=26 / 61.54% / +0.0567%` | `n=59 / 35.59% / -0.0192%` |

这比只比较两个策略的总体均值更有解释力：同一批原候选中，被状态 gate veto 的部分在两股
都为负，被保留部分在两股都为正。因此 `max_round_trips` 不是问题核心，第一阶段 abstain
确实改变了点位质量。但长江电力 5 分钟毛均值只有 0.0567%，仍不能称为有交易经济性。

SELL 的透明 causal baseline 定义为：

```text
return_15 < 0 AND return_30 < 0
AND close < session_vwap
AND abs((close - ema_slow) / ATR) <= 0.5
AND recent_volume_ratio_max >= 0.8
AND recent_15_bar_high / recent_15_bar_low - 1 >= 0.5%
AND MACD rebound momentum starts to roll over
AND current bar is bearish and closes below previous close
```

`return_15/30` 和 16 点 high/low 区间只在当日 session 内滚动；未来 bar 不参与候选。业务
解释是：先确认 15/30 分钟均处于下行，只在价格反弹靠近慢 EMA、但仍受 session VWAP 压制
且 MACD 反弹动能开始回落时寻找倒 T SELL；低波动窗口直接 `no_signal`。这不是 BUY 条件乘
以 -1，也不在已经深跌远离均线时追卖。
...
| 股票/side | 5 分钟 | 15 分钟 |
|---|---:|---:|
| 生益科技 BUY | `n=29 / 55.17% / +0.3059% / median +0.1491%` | `n=29 / 55.17% / -0.0317% / median +0.0926%` |
| 生益科技 SELL | `n=54 / 62.96% / +0.1090% / median +0.1141%` | `n=52 / 71.15% / +0.2773% / median +0.4204%` |
| 长江电力 BUY | `n=26 / 61.54% / +0.0567% / median +0.0696%` | `n=26 / 69.23% / +0.1501% / median +0.1232%` |
| 长江电力 SELL | `n=12 / 75.00% / +0.1393% / median +0.1053%` | `n=12 / 41.67% / +0.0536% / median -0.0524%` |

机会门槛敏感性也保留了失败结果。无振幅门槛时长江电力 SELL 有 35 个，15 分钟均值
`-0.0156%`；0.5% 门槛降到 12 个并改善 5 分钟；1% 只剩 2 个，1.5% 为 0，不能据此宣称
更高阈值更优。生益科技从无门槛 56 个到 0.5% 的 54 个，15 分钟正确率从 69.81% 微升到
71.15%，没有为迁就低波动股牺牲主要样本。

对“开盘更有效”的检查不支持全局时段硬编码：生益科技旧 BUY 在 09:30–10:00 为
`n=18、61.11%、+0.3955%`，新 BUY 为 `n=7、71.43%、+1.0151%`；但长江电力旧 BUY 开盘
均值 `-0.0124%`，新 BUY 的开盘、上午、下午 5 分钟均值分别只有
`+0.0581%/+0.0711%/+0.0450%`，没有开盘独占优势。更合理的解释是开盘价格发现提高了生益
科技的机会幅度，而不是“10 点以后策略必然失效”。时段应作为 Stage-1 的条件特征，并与
同分钟历史波动/量能比较，不能固定关闭午后。

失败日同样必须保留。生益科技 8 月 13 日新 BUY 为 0，解决了连续抄底；但独立 SELL 在
10:57、13:21、14:04 发出 3 次，未来 5 分钟方向收益分别为
`-0.8227%/-0.1907%/-0.1651%`，全部错误。8 月 14 日的 11:22 SELL 正确（5/15 分钟
`+0.4290%/+1.2869%`），13:22 和 14:45 SELL 错误；14:25 BUY 只在 5 分钟正确、15 分钟
为 `-1.3033%`。所以“总体均值改善”不等于对用户关注的两个单边日已经给出漂亮点位。

进一步要求 MACD 红柱连续两根收敛，会把生益科技 SELL 压到 13 个并把 5 分钟均值提高到
`+0.2879%`，但长江电力只剩 3 个且均值转成 `-0.0592%`；连续三根在两股合计只剩 4 个。
该规则表现出明显单股选择偏差，因此未进入默认实现。下一步不是围绕 8 月 13 日继续增加
补丁，而是按第 4–6 章使用跨股票状态、市场残差、同分钟机会基线和冻结 walk-forward 去判断
这些反弹究竟是下跌中继还是可做的回落。
