...
| timeframe | string | 否 | daily | K 线周期 |
...
后端内部流程：
1. 通过 PhoenixAClient 获取 OHLCV 数据
...
```
1. 页面加载
   → GET /workbench/market-data?symbol=xxx&start_date=...&end_date=...
   → 拿到纯 bars 数据
...
2. 用户选择指标（从指标选择器中点选）
   → POST /workbench/indicators
     {symbol, start_date, end_date, indicators: [{name: "sma", params: {period: 10}}]}
