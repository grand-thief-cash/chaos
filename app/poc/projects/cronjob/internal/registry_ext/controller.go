...
import (
	...
	"github.com/grand-thief-cash/chaos/app/poc/projects/cronjob/internal/api"
)

func init() {
	// Ensure http_server starts after our controller component by extending its runtime dep graph.
	registry.ExtendRuntimeDependencies(appconsts.COMPONENT_HTTP_SERVER, api.NewPOCController().Name())

	registry.RegisterAuto(func(cfg *config.AppConfig, c *core.Container) (bool, core.Component, error) {
		return true, api.NewPOCController(), nil
	})
}
